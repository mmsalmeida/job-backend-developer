# O Desafio
Atender as requisições de uma aplicação alto índice de acesso (transações/requisições).
O caminho para sua solução.
Utilizando das tecnologias do Java 8 e do Spring boot, o pequeno projeto a ser apresentado utilizou também:  
* JPA/ H2/ DevTools/ Eureka/ Actuator.

## O projeto foi divido em três aplicações:
* **Gateway** – a parte que fica exposta ao cliente, além distribuições requisições e gerenciá-las.
...
Banco de dados: configurado junto ao login-service depois que a aplicação estiver no ar podemos entrar no console do mesmo afim de verificar a existência da tabela de 
usuário que contém as informações de id/email/login/nome/senha
...
com a necessidade de suas requisições onde caso necessário realizar a distribuição para novas máquinas,
utilizando o discovery como ponto de gerenciamento de dados.

O discovery e o gateway possibilitam que a aplicação possa ser distribuida de forma horizontal tornando
...
- Utilização e verificações de ferramentas onde seria possível aplicar mais testes de "stress" e tempos de reposta do serviço também não foram utilizadas.
- Por se tratar de uma aplicação simples para back-end não foi dada ênfase em sua parte de amostragem já que o intuito era apenas mostrar sua funcionalidade.
